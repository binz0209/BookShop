...
public class DAO extends DBContext {
    ...
    public List<Products> getTop3Products() {
        List<Products> p = new ArrayList<>();
        String sql = "SELECT TOP 3 * FROM Products WHERE isContinue <> 1";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public boolean updateProduct(int id, long price, int quantity, int cateID, int statusProduct) {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            ps.setBoolean(4, statusProduct != 0);
            ...
            System.out.println("updateProduct -> rows=" + rows);



            try (PreparedStatement ps2 = con.prepareStatement(

                    "SELECT CategoryID, isContinue, Price, Quantity FROM Products WHERE ProductID=?")) {
                ...
                try (ResultSet rs = ps2.executeQuery()) {
                    ...
                    if (rs.next()) {
                        ...
                        System.out.printf("DB after update -> cateID=%d, isContinue=%s, price=%d, qty=%d%n",

                                rs.getInt("CategoryID"),

                                rs.getBoolean("isContinue"),
    ...
    public List<OrderHistory> getOrderHistory() {
        ...
        //thêm
        public List<Products> filterProducts(Integer categoryId, String priceRange) {
        ...
        if (priceRange != null) {
            switch (priceRange) {
                case "low":
                    ...
                    break;
                case "mid":
                    sql += " AND Price BETWEEN 500000 AND 2000000";
                    break;
                case "high":
        ...
        while (rs.next()) {
            ...
            status = "Error at filterProducts " + e.getMessage();
        }
        return list;
    }

    //thêm
    public List<Products> getBestOptionProducts() {
        List<Products> list = new ArrayList<>();
        String sql = """
            SELECT TOP 5 p.*
            FROM Products p
            JOIN OrderDetails od ON p.ProductID = od.ProductID
            GROUP BY p.ProductID, p.ProductName, p.CategoryID, p.Description, p.Price, p.Image, p.Quantity, p.isContinue
        ...
        while (rs.next()) {
            ...
            status = "Error at getBestOptionProducts " + e.getMessage();
        }
        return list;
    }
